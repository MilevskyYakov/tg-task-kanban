...
function App() {
  ...
  const board = navigation.screen === 'board' ? boards.find((item) => item.id === navigation.boardId) : undefined;
  ...
  useEffect(() => {
    ...
    void api('/api/auth/telegram', json('POST', {initData: webApp.initData}))
      .then(async (auth) => {
        ...
        if (token) { const board = await api<Board>('/api/board-links/redeem', json('POST', {token})); setNavigation({ screen: 'board', boardId: board.id }); }
  ...
  useEffect(() => {
    ...
    if (navigation.screen === 'tasks') void api<{tasks: Task[]}>('/api/tasks/mine').then((data) => setTasks(data.tasks)).catch((error: Error) => setMessage(error.message));
  ...
  if (navigation.screen === 'tasks') return <AppShell message={message} navigation={navigation} navigate={navigate}><TasksScreen>{taskList}{!tasks.length && <p>Назначенных задач пока нет.</p>}</TasksScreen></AppShell>;
