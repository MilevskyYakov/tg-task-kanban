...
import { activeFilterCount, dateInputToIso, defaultFilters, filterTasks, optimisticUpdate, resolveTaskBoard, type Task, type TaskFilters, type TaskStatus } from './tasks';
...
function App() {
  ...
  const taskToolbar = <>
    ...
    {filterCount > 0 && <div className="filter-chips" aria-label="Активные фильтры">
      ...
      {filters.status && <button onClick={() => setFilter('status', '')}>{statusName[filters.status]} ×</button>}
  ...
  const filterSheet = showFilterSheet && <Sheet title="Фильтры" onClose={() => setShowFilterSheet(false)}>
    <div className="filter-sheet">
      ...
      <FieldRow label="Статус"><select value={filters.status} onChange={(event) => setFilter('status', event.target.value as TaskFilters['status'])}><option value="">Без завершённых</option>{statuses.map((status) => <option key={status} value={status}>{statusName[status]}</option>)}</select></FieldRow>
