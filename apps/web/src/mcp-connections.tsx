...
export function McpConnections({navigate}: {navigate: (next: NavigationState) => void}) {
  ...
  const openHelp = () => { setMessage(''); setError(''); setDialog('help'); };
  const connectCommand = `hermes mcp add task_kanban --url '${serverUrl.replace(/'/g, "'\\''")}' --auth header`;
  ...
  const body = <>
    ...
    {stage === 'secret' && <div className="mcp-secret">{summary}<h2>Сохраните ключ сейчас</h2><p>После выхода повторный показ недоступен. Вставьте ключ в настройки клиента, не в переписку с агентом.</p>{address}<p className="mcp-label">Ключ доступа</p><p className="mcp-code mcp-key" tabIndex={0}>{secret}</p><p>Ключ действует до отзыва. При утечке отзовите подключение.</p><button className="mcp-help back" onClick={openHelp}>Как подключить Hermes</button><div className="mcp-actions"><button onClick={()=>void copy(secret,'key')}>{copied ? 'Ключ скопирован' : 'Копировать ключ'}</button><button className="secondary" onClick={back}>Готово</button></div></div>}
  ...
  return <AppShell message="" navigation={{screen:'settings-connections'}} navigate={navigate} hideNavigation={isolated || expired}>
    ...
    {!expired && dialog === 'help' && <Sheet title="Как подключить Hermes" onClose={()=>setDialog(undefined)}><div className="mcp-sheet">
      ...
      <ol className="mcp-help-steps">
        <li><p>Скопируйте и выполните команду. Адрес уже подставлен — ничего заменять не нужно.</p><pre className="mcp-code mcp-command" tabIndex={0}><code>{connectCommand}</code></pre><button className="secondary" onClick={()=>void copy(connectCommand,'command')}>Копировать команду подключения</button></li>
        ...
        <li><p>Проверьте подключение:</p><pre className="mcp-code mcp-command" tabIndex={0}><code>{testCommand}</code></pre><button className="secondary" onClick={()=>void copy(testCommand,'command')}>Копировать команду проверки</button></li>
      </ol>
      ...
      <p>Ключ вводите только в скрытом запросе терминала, не в команду или чат. Его повторный показ в приложении недоступен.</p>
      {error && <p role="alert">{error}</p>}{message && <p role="status">{message}</p>}
      <a href="https://hermes-agent.nousresearch.com/docs/user-guide/features/mcp" target="_blank" rel="noopener noreferrer">Документация Hermes</a><button onClick={()=>setDialog(undefined)}>Готово</button>
    </div></Sheet>}
  </AppShell>;
}
