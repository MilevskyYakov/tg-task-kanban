...
- Первый пакет опубликован и влит через PR #84 по отдельной команде `/taskfinish`. После предъявления совмещённого сообщения владелец ответил «OOk что дальше», затем «Го» на предложение опубликовать последнюю композицию через PR и закрыть #76. Это завершение дизайна, не запуск кода #77 или deploy.
...
**Совмещённый макет:** `Mt93Z` (390 px) / `a8ltz` (320 px) содержат общую иллюстрацию над подписью и одну кнопку в том же сообщении. Обновлены оба PNG и обзор подключения. Белый блок использует вертикальную компоновку: отправитель, изображение, заголовок, описание, кнопка; на 320 px заголовок переносится, текст и действие не обрезаются. Собранная композиция принята после предъявления; metadata — `approved`. Это макет Telegram-чата, не реальная отправка ботом и не Telegram smoke.
...
- Старые roots относительно базового commit сохранены без изменений. Все 73 принятых root отмечены `approved`. Название `REVIEW` сохранено для стабильности идентификаторов.
...
## Карта наблюдений и переходов для design acceptance

Это целевой разбор принятых сценариев, не полный аудит приложения. Источники продуктовых решений — #77–#80 и ответы владельца; индекс ниже связывает каждый сценарий с реальным root/PNG.

- **Создание (#77):** в `apps/web/src/main.tsx:610–623` форма показывает дату/время, но не выбор начального статуса. Новый путь: `create390` / `create320`, выбор `date390` / `time320` / `status390`, при блокере — `blocker320` / `blockerTask390`, затем `afterCreate390`. Ошибка, ожидание и запрет завершения представлены отдельно. Это согласованный новый UX, не утверждение о выполненной API-реализации.
- **Сбор и разбор (#78):** `apps/web/src/tasks.ts:182–187` исключает неназначенные из «Мои»; «Готово» требует явного фильтра. Путь: заметный вход в `backlog390` / `backlog320`, `claim390`, затем успех либо конфликт. `allUnassigned390` показывает неназначенные других статусов через «Все», не меняя их статус; архив отдельно. Повторное создание — `repeat320`, вставка — `bulkInput390`, предпросмотр, частичная ошибка и успех. Сохранность данных и атомарность проверяются в реализации #78.
- **Пара (#79):** выбранный владельцем новый сценарий, а не доказанный дефект существующего UI. Путь: `pair_entry390`, `pair_create390`, приглашение владельца, `pair_inviteAccept390`, `pair_joined390`. Доступ, выход, отзыв, замена участника и архив представлены отдельными roots с отказами/ошибками. Решение об обнулении только исполнителя и сохранении истории закреплено выше.
- **Групповой вход (#80):** исправление предложенного сценария по ответу владельца — одно правильное сообщение, без команд обновления. Путь: `group_guide390`, `group_picker320`, `group_message390` / `group_pinned320`, первичная настройка/ожидание, затем пустой список; повторный вход — `group_return390` / `group_return320`. Ошибка, загрузка и замороженная доска имеют отдельные roots. Картинка встроена в оба макета сообщения, а не оставлена только самостоятельным asset.
- **Второстепенные поверхности (#81):** browser prompts подтверждены в `apps/web/src/main.tsx:267–274`, но владелец отклонил переделку. Это явное исключение из нового design scope, не исправленный дефект и не скрытый TBD.

### Доступность и граница приёмки

Для совмещённого сообщения рассчитан контраст sRGB: основной текст `#172033` на белом — 16.27:1; кнопка `#0756CB` на `#EDF4FF` — 5.93:1; отправитель `#0756CB` на белом — 6.56:1. Все три пары выше 4.5:1. Это проверка указанных текстовых пар, не полный accessibility-аудит приложения.

Сохранены требования `visual-contract-v2.md`: поля 18/12 px, safe areas, цели нажатия от 44 px, перенос текста, доступная клавиатурная навигация, focus trap/Escape/возврат фокуса для sheets. В новом сообщении кнопка имеет высоту 52 px, подпись остаётся текстом, а иллюстрация не заменяет смысл или название действия. Макеты создания с клавиатурой, ожиданием и ошибкой перечислены в индексе. Проверка реального фокуса, экранного диктора, масштабирования текста и Telegram iOS/Android остаётся у implementation children и #83: статический PNG её не доказывает. Блокирующих вопросов по композиции/текстам нет; техническая проверка date-only из #77 и отдельный security-дизайн #82 не выдаются за завершённые этим handoff.
